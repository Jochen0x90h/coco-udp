...
Coroutine sender(Loop &loop, Buffer &buffer) {
	...
	while (true) {
		...
		debug::toggleRed();
#ifdef NATIVE
		std::cout << "Sent " << buffer.transferred() << " to port " << buffer.getHeader<ipv6::Endpoint>().port << std::endl;
#endif
...
Coroutine receiver(Loop &loop, Buffer &buffer) {
	while (true) {
		...
		debug::toggleGreen();
#ifdef NATIVE
		std::cout << "Received " << buffer.transferred() << " from port " << buffer.getHeader<ipv6::Endpoint>().port << std::endl;
#endif
	}
}
